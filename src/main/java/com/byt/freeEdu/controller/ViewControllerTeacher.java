...
public class ViewControllerTeacher{
  ...
  public Mono<String> getScheduleTeacher(Model model) {
    return sessionService.getUserId().map(userId -> {
  ...
  public Mono<String> addRemark(@ModelAttribute RemarkDto remarkDto) {
    return sessionService.getUserId().flatMap(teacherId -> {
  ...
  public Mono<String> editRemarkForm(@PathVariable int remarkId, Model model) {
    return sessionService.getUserId().flatMap(userId -> {
      ...
      StudentDto studentDto = new StudentDto(user.getUserId(), user.getFirstname(),
          user.getLastname());
  ...
  public Mono<String> editGrade(@PathVariable int gradeId, @ModelAttribute GradeDto gradeDto,
      Model model) {
    return sessionService.getUserId().flatMap(userId -> {
      try {
  ...
  public Mono<String> markAttendance(@PathVariable int classId, @RequestParam String subject,
      Model model) {
    return sessionService.getUserId().flatMap(userId -> {
      ...
      model.addAttribute("students",students);
      model.addAttribute("selectedSubject",SubjectEnum.valueOf(subject)); // Konwersja na
      // SubjectEnum
  ...
  public Mono<String> saveAttendance(@ModelAttribute AttendanceFormDto attendanceFormDto,
      Model model) {
    return sessionService.getUserId().flatMap(userId -> {
      if (attendanceFormDto.getAttendanceMap().isEmpty()
          || attendanceFormDto.getGlobalSubject() == null) {
        model.addAttribute("errorMessage","Nie zaznaczono wszystkich wymaganych danych.");
        return Mono.just("teacher/teacher_attendanceMark");
      }

      attendanceService.markAttendance(attendanceFormDto.getAttendanceMap(),
          attendanceFormDto.getGlobalSubject(),userId);
