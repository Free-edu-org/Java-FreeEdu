...
public interface ScheduleMapper{
    ...
    @Mapping(target = "teacher", ignore = true)      // ustawiane w serwisie
    @Mapping(target = "schoolClass", ignore = true)  // ustawiane w serwisie
    Schedule toEntity(ScheduleDto dto);

    @Mapping(target = "scheduleId", ignore = true)   // ID nie zmieniamy przy update
    @Mapping(target = "subject", source = "subjectName", qualifiedByName = "mapSubject")
    @Mapping(target = "teacher", ignore = true)      // relacje ustaw w serwisie
    ...
    default SubjectEnum mapSubject(String subjectName) {
        ...
        switch (subjectName) {
            case "Język polski": return SubjectEnum.POLISH;
            case "Matematyka": return SubjectEnum.MATH;
            case "Geografia": return SubjectEnum.GEOGRAPHY;
            case "Historia": return SubjectEnum.HISTORY;
            case "Nauki ścisłe": return SubjectEnum.SCIENCE;
            case "Sztuka": return SubjectEnum.ART;
            case "Wychowanie fizyczne": return SubjectEnum.SPORTS;
            default: return null;
        }
    }
